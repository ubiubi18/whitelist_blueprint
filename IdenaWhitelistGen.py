...
def main():
    ...
    collect_shortsession_addresses(target_epoch)

    _, ep_data = get_epoch_info(last_epoch)
    stake_threshold = float(ep_data.get("discriminationStakeThreshold", 0))
    print(f"\nDiscriminationStakeThreshold: {stake_threshold}")

    bad_addrs = fetch_bad_addresses(target_epoch)
    ...
    for i, addr in enumerate(addresses):
        ...
        try:
            vs_url = f"https://api.idena.io/api/Epoch/{target_epoch}/Identity/{al}/ValidationSummary"
    ...
    with open(WHITELIST_LIST_FILE, "w") as f:
        f.write(",\n".join(whitelist))

    with open(WHITELIST_META_FILE, "w") as f:
        json.dump({"DiscriminationStakeThreshold": stake_threshold, "epoch": target_epoch}, f)

    root, _ = build_merkle_root(whitelist)
    print("\nMERKLE_ROOT =", root)
    with open(f"merkle_root_epoch_{target_epoch}.txt", "w") as f:
        f.write(root + "\n")

    with open(WHITELIST_META_FILE, "r+") as f:
        meta = json.load(f)
